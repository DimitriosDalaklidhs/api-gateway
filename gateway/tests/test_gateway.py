...
def mock_redis():
    ...
    class FakeRedis:
        ...
        async def set(self, key, value): store[key] = value; return True
        async def setex(self, key, ttl, value): store[key] = value; ttls[key] = ttl; return True
        async def incr(self, key): store[key] = str(int(store.get(key, "0")) + 1); return int(store[key])
        async def expire(self, key, ttl): ttls[key] = ttl; return True
        async def exists(self, key): return 1 if key in store else 0
        async def delete(self, *keys):
            for k in keys:
                store.pop(k, None)
            return len(keys)
        async def keys(self, pattern="*"): return [k for k in store if k.startswith(pattern.replace("*", ""))]
        async def ttl(self, key): return ttls.get(key, -1)
        async def script_load(self, script): scripts["loaded"] = script; return "abc123"
        async def evalsha(self, sha, numkeys, *args):
            # Simulate INCR + EXPIRE logic from _INCR_SCRIPT
            key = args[0]
        ...
        def pipeline(self):
            class Pipe:
                ...
                def ttl(self, key): self._cmds.append(("ttl", key)); return self
                async def execute(self):
                    results = []
                    for cmd, key in self._cmds:
                        if cmd == "get": results.append(store.get(key))
                        elif cmd == "ttl": results.append(ttls.get(key, -1))
                    return results
            return Pipe()
...
class TestCircuitBreaker:
    ...
    async def test_raises_when_open(self, mock_redis):
        ...
        # Set opened_at to recent time so it stays OPEN
        import time
...
class TestProxy:
    ...
    async def test_forwards_200(self):
        ...
        fake_response = HttpxResponse(200, content=b'{"ok":true}', headers={"content-type": "application/json"})
...
class TestIntegration:
    """Light integration tests using TestClient (sync httpx calls)."""

    def setup_method(self):
        """Patch Redis and downstream before each test."""
        pass
